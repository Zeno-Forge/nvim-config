return {
    ...
    config = function ()
        ...
        local c = {
          ...
          provider = function()
            local os_icon
            local sys_os = vim.loop.os_uname().sysname
            if sys_os == "Darwin" then  -- macOS
              os_icon = icons["mac"]
            elseif sys_os == "Linux" then  -- Linux
              os_icon = icons["unix"]
            else  -- Other OS or unable to detect
              os_icon = icons[vim.bo.fileformat] or ""
            end
            return fmt(" %s %s ", os_icon, vim.bo.fileencoding)
          end,
            hl = "StatusLine",
